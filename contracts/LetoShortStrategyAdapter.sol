...
contract LetoShortStrategyAdapter {
	...
	function manualWithdraw(uint256 _amount) external returns (uint256) {
		...
		pool.lendingAdapter().withdraw(address(bid_token), _amount, address(pool));

		emit ManualWithdraw(address(bid_token), _amount);
	...
	function withdraw(ILetoPool pool, PoolState memory state) private returns (uint256 amountOut) {
		ILetoToken asset0 = ILetoToken(pool.asset0());
		...
		uint256 _amount_converted = (amount * (10 ** (asset0.decimals()))) / uint256(pool.latestPairPrice());

		pool.lendingAdapter().withdraw(address(state.parameters.asset0), _amount_converted, address(pool));
		amountOut = pool.swap(state.parameters.asset0, state.parameters.asset1, _amount_converted, 0); // FIXME: calculate minimal amount out
		pool.repay(state.parameters.asset1, amountOut);

		emit RebalanceWithdrawalStep(state.parameters.asset1, _amount_converted);
	}
	...
	function borrow(ILetoPool pool, PoolState memory state) private returns (bool) {
		...
		pool.borrow(state.parameters.asset1, rebalancingAmount_);

		emit RebalanceBorrowStep(state.parameters.asset1, rebalancingAmount_);
	}
	...
	function poolState(address pool_) public view returns (PoolState memory) {
		...
		uint256 borrowedAssets_ = borrowed(pool_, asset1.symbol());
	...
	function price(address pool_) public view returns (uint256) {
		...
		uint256 deltaPrice = (uint256(pool.initialPairPrice()) * decimals_) / uint256(pool.latestPairPrice());
